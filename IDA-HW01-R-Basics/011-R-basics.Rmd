...
```{r}
boolean_vector = str_detect(names(passed), "J")
names = names(passed)[boolean_vector]
names
```
...
```{r}
even_cumulative_mean = function(a) {
     if (length(a)<= 1 || is.numeric(a) == FALSE){
         stop("Error in 'new_applier_function': input vector has length>=1 or is not numerical.")}
    indices = 1:length(a)
    t = tibble(
    even_indices = indices[indices%%2<=0],
    entries = a[even_indices],
    # 3. task is missing
    )}
```
...
```{r}
 numbers <- c(gsub("[^[:digit:].]", "", weird_RTs)) %>%
     as.numeric %>%
      .[. >= 100]  %>%
     log %>%
     mean %>%
